...
export async function updateUserProfile(userId: string, updatedData: IUpdateUserProfile) {
  if (!userId) throw new Error("User ID is required");

  try {
    // Updating the user profile document in the Appwrite database
    const response = await databases.updateDocument(
      appwriteConfig.databaseId,        // The database ID
      appwriteConfig.userCollectionId,   // The user collection ID where profiles are stored
      userId,                            // The document ID (user ID)
      updatedData                        // The updated profile data
    );

    return response;  // Return the updated user profile data
  } catch (error) {
