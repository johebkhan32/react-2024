...
export const useUpdateUserProfile = (userId: string) => {
  ...
  return useMutation({
    mutationFn: (updatedData: IUpdateUserProfile) => updateUserProfile(userId, updatedData),
    onSuccess: (data) => {
      console.log(data)
      // Assuming `data` contains the updated user profile data
      // Invalidate the query for the current user to refresh the profile data
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_CURRENT_USER],
      });

      // Invalidate the query for the user by their ID, to update the profile page
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_USER_BY_ID, userId],
      });
    },
